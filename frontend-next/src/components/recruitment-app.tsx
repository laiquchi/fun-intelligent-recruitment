...
const NAV_ITEMS: Array<{ key: ViewKey; label: string; children?: Array<{ key: ViewKey; label: string }> }> = [
  { key: "social-recruiting", label: "招聘中" },
  { key: "social-completed", label: "已完成岗位" },
  {
    key: "social-interview-board",
    label: "查看面试安排",
    children: [
...
export function RecruitmentApp({ onLogout }: RecruitmentAppProps) {
  ...
  const [showManualInterviewForm, setShowManualInterviewForm] = useState(false);
    ...
    try {
      ...
      const normalized = (response.data || [])
        ...
        .sort((a, b) => {
          ...
          const typeOrder: Record<string, number> = { 社招: 3, 校招: 2, 实习生: 1 };
          const priorityDiff = (priorityOrder[b.priority || "medium"] || 0) - (priorityOrder[a.priority || "medium"] || 0);
          if (priorityDiff !== 0) return priorityDiff;
          const aType = a.recruitmentType || "";
          const bType = b.recruitmentType || "";
          const typeDiff = (typeOrder[bType] || 0) - (typeOrder[aType] || 0);
          return typeDiff;
        });
      setItems(normalized);
    } catch (error) {
  ...
  const recruitingItems = useMemo(() => {
    const filtered = items
      ...
      .filter((item) => (item.department || "").toLowerCase().includes(departmentFilter.toLowerCase()));
    return filtered;
  }, [items, positionFilter, departmentFilter]);
              ...
              <TableBody>
                {recruitingItems.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="max-w-[300px] truncate" title={item.remarks || "-"}>{item.remarks || "-"}</TableCell>
                    <TableCell>
  ...
  const renderInterviewPassedView = () => {
    return (
      <Card className="border-slate-200/70 bg-white/90 shadow-lg shadow-cyan-900/5 backdrop-blur">
        <CardHeader className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <CardContent>
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            <Table>
              ...
              <TableBody>
                {filteredPassedInterviews.map((item) => (
                  <TableRow
                    key={item.id}
        ...
        <Card className="border-slate-200/70 bg-white/90 shadow-lg shadow-cyan-900/5 backdrop-blur">
          <CardHeader className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setShowManualInterviewForm(!showManualInterviewForm)}
            >
              {showManualInterviewForm ? "收起" : "展开"}
            </Button>
          </CardHeader>
          {showManualInterviewForm && (
          ...
          <CardHeader className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <CardDescription>拖拽或粘贴面试安排截图，自动提取候选人信息</CardDescription>
              </div>
              <div
                className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-input px-3 py-2 text-sm"
                onDrop={(event) => {
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_20%_10%,rgba(8,145,178,0.14),transparent_35%),radial-gradient(circle_at_80%_0%,rgba(234,88,12,0.11),transparent_32%),linear-gradient(180deg,#f8fbfc_0%,#f6f8fa_42%,#eef3f7_100%)]">
      <div className="mx-auto grid min-h-screen max-w-[1700px] grid-cols-1 gap-4 p-4 lg:grid-cols-[260px_1fr] lg:p-6">
              ...
              <div key={item.key}>
                {item.children ? (
                  <div className="space-y-1">
                    <button
                      type="button"
                      onClick={() => setCurrentView(item.children[0].key)}
                      className={cn(
                      ...
                      <div className="ml-4 mt-1 space-y-1">
                        {item.children.map((child) => (
                          <button
                            key={child.key}
                            type="button"
                            onClick={() => setCurrentView(child.key)}
                            className={cn(
                          ...
                          >
                            ...
                            {currentView === child.key && <span className="h-1.5 w-1.5 rounded-full bg-cyan-600" />}
                          </button>
                        ))}
                      </div>
                    )}
        ...
        <main className="space-y-4">
          ...
          {currentView === "social-interview-passed" && renderInterviewPassedView()}
            ...
            <Field label="招聘人数">
              <Input
                ...
                onChange={(event) => {
                  ...
                  if (value === "") {
                    setRecruitmentForm((prev) => ({ ...prev, count: "" as unknown as number }));
                  } else {
                    const num = parseInt(value);
                    setRecruitmentForm((prev) => ({ ...prev, count: isNaN(num) ? 1 : num }));
                  }
                }}
                onBlur={(event) => {
                  const value = event.target.value;
                  const num = parseInt(value);
                  setRecruitmentForm((prev) => ({ ...prev, count: isNaN(num) || num < 1 ? 1 : num }));
                }}
              />
            </Field>
            <Field label="面试官">
