...
function normalizeInterviewRecord(item) {
  ...
  return [
    ...
    item.remarks ?? null,
    item.feedback ?? null,
    item.productSense ?? null,
    item.systemDesign ?? null,
    item.agentAbility ?? null,
    item.communication ?? null,
    item.ownership ?? null,
    item.learningSpeed ?? null,
...
function mapInterviewRow(row) {
  return {
    ...
    productSense: row.productSense == null ? null : toNumber(row.productSense, 0),
    systemDesign: row.systemDesign == null ? null : toNumber(row.systemDesign, 0),
    agentAbility: row.agentAbility == null ? null : toNumber(row.agentAbility, 0),
    communication: row.communication == null ? null : toNumber(row.communication, 0),
    ownership: row.ownership == null ? null : toNumber(row.ownership, 0),
    learningSpeed: row.learningSpeed == null ? null : toNumber(row.learningSpeed, 0),
...
async function initDatabase() {
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS interviews (
      ...
      product_sense INT NULL,
      system_design INT NULL,
      agent_ability INT NULL,
...
async function readInterviews() {
  ...
  const [rows] = await db.query(`
    SELECT
      ...
      remarks,
      feedback,
      product_sense AS productSense,
      system_design AS systemDesign,
      agent_ability AS agentAbility,
      communication,
      ownership,
      learning_speed AS learningSpeed,
